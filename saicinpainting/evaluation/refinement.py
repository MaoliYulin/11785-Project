...
def _pyrdown_mask(mask: torch.Tensor, downsize: tuple = None, eps: float = 1e-8, blur_mask: bool = True,
                  round_up: bool = True):
...
def pixel_refine_after_R(
    ...
    assert batch["image"].shape[0] == 1, "pixel_refine_after_R 只支持 batch_size=1"
...
def refine_predict(
    ...
    for ids, (image, mask) in enumerate(zip(ls_images, ls_masks)):
        ...
        image_inpainted = _infer(
            ...
            lambda_edge=lambda_edge if lambda_edge is not None else 1.0,
